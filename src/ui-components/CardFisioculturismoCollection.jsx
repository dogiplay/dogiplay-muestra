...
import { listAtletas } from "../graphql/queries";
import CardFisioculturismo from "./CardFisioculturismo";
...
export default function CardFisioculturismoCollection(props) {
  ...
  const loadPage = async (page) => {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const variables = {
        limit: pageSize,
        filter: { disciplina: { eq: "fisicoculturismo" } },
      };
      if (newNext) {
      ...
      const result = (
        await client.graphql({
          query: listAtletas.replaceAll("__typename", ""),
          variables,
        })
      ).data.listAtletas;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "CardFisioculturismoCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
