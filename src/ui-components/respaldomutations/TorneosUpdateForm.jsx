...
import { getTorneos as getTorneosMutations} from "../graphql/queries";
import { updateTorneos as updateTorneosMutations} from "../graphql/mutations";
const client = generateClient();
export default function TorneosUpdateForm(props) {
  const {
    id: idProp,
    torneos: torneosModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = torneosRecord
      ? { ...initialValues, ...torneosRecord }
      : initialValues;
    setIdliga(cleanValues.idliga);
    setIddeporte(cleanValues.iddeporte);
    setIdtorneo(cleanValues.idtorneo);
    setNombretorneo(cleanValues.nombretorneo);
    setNombredeporte(cleanValues.nombredeporte);
    setErrors({});
  };
  const [torneosRecord, setTorneosRecord] = React.useState(torneosModelProp);
  React.useEffect(() => {
    ...
    queryData();
  }, [idProp, torneosModelProp]);
  React.useEffect(resetStateValues, [torneosRecord]);
  const validations = {
    idliga: [],
    iddeporte: [],
    idtorneo: [],
  ...
  return (
    <Grid
      ...
      {...getOverrideProps(overrides, "TorneosUpdateForm")}
      {...rest}
    >
      <TextField
        label="Idliga"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || torneosModelProp)}
