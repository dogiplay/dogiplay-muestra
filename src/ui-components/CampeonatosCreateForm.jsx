...
import { createCampeonatos } from "../graphql/mutations";
const client = generateClient();
export default function CampeonatosCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setTemporada(initialValues.temporada);
    setNumero_campeonato(initialValues.numero_campeonato);
    setCategoria(initialValues.categoria);
    setAnoc(initialValues.anoc);
    setErrors({});
  };
  const validations = {
    clave_liga: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createCampeonatos.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "CampeonatosCreateForm")}
      {...rest}
    >
      <TextField
        label="Clave liga"
